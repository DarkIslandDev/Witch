...
public class ItemPlacementHelper
{
    ...
    public ItemPlacementHelper(IEnumerable<Vector2Int> roomFloor, HashSet<Vector2Int> roomFloorNoCorridor)
    {
        ...
        foreach (Vector2Int position in roomFloorNoCorridor)
        {
            ...
            if (type == PlacementType.NearWall &&
                graph.GetNeighbours4Directions(position).Count < neighboursCount8Dir) continue;
        ...
        bool addOffset)
    {
        ...
        while (iteration < iterationsMax)
        {
            ...
            if (itemArea > 1)
            {
                (bool result, List<Vector2Int> placementPositions) = PlaceBigItem(position, size, addOffset);
