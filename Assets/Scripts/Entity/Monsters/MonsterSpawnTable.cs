...
public class MonsterSpawnTable
{
    ...
    public float GetSpawnRate(float t)
    {
        ...
        for (int i = 1; i < spawnRateKeyframes.Length; i++)
        {
            if (spawnRateKeyframes[i].t >= t)
                return Mathf.Lerp(spawnRateKeyframes[i - 1].spawnRate, spawnRateKeyframes[i].spawnRate,
                    Remap01(spawnRateKeyframes[i - 1].t, spawnRateKeyframes[i].t, t));
        }
    ...
    private int SelectMonster(int i, float t)
    {
        ...
        float cumulative = 0;
        float tLerp = Remap01(spawnChanceKeyframes[i].t, spawnChanceKeyframes[i].t, t);
        for (int j = 0; j < spawnChanceKeyframes[i].spawnChances.Length; j++)
        {
            cumulative += Mathf.Lerp(spawnChanceKeyframes[i].spawnChances[j],
                spawnChanceKeyframes[i].spawnChances[j], tLerp);
            if (rand < cumulative)
                return j;
        }
    ...
    {
        float tLerp = Remap01(hpMultiplierKeyframes[i].t, hpMultiplierKeyframes[i].t, t);
        return Mathf.Lerp(hpMultiplierKeyframes[i].healthBuffs[j], hpMultiplierKeyframes[i].healthBuffs[j], tLerp);
    }
